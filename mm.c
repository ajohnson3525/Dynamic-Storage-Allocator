 ...
 *
 * Name(s): AJ Johnson, Eliot Kopf 
 * NetID(s): aej3525, elsakopf
 *
 * NOTES:
...
void* mm_malloc(size_t size) {
  ...
  // Implement mm_malloc.  You can change or remove any of the
  ...
  // check that a block was returned - if not, allocate more space in the heap
  if (ptr_free_block == NULL) {
     request_more_space(req_size);
     ptr_free_block = search_free_list(req_size);
  }

  // remove that block from free list (this updates the ptrs of the block for us)
  remove_free_block(ptr_free_block);

  // find block_size
  block_size = SIZE(ptr_free_block->size_and_tags);

  // split block to avoid internal fragmentation
  split_size = block_size - req_size;

  if (split_size >= MIN_BLOCK_SIZE) {	  
     block_info* split_block = UNSCALED_POINTER_ADD(ptr_free_block, req_size); 
          
     // update size_and_tags of the block that was just added
     // update footer as well
     split_block->size_and_tags = (split_size | (TAG_PRECEDING_USED)) & (~TAG_USED);
     *((size_t*) UNSCALED_POINTER_ADD(split_block, (split_size - WORD_SIZE))) = split_block->size_and_tags; 

     // insert the new block (this changes ptrs for us)
     insert_free_block(split_block);

     // update size_and_tags of block just allocated
     ptr_free_block->size_and_tags =
             req_size | TAG_USED | ((ptr_free_block->size_and_tags) & TAG_PRECEDING_USED);
  } else {
     // update prev used tag of following block
     ((block_info*)UNSCALED_POINTER_ADD(ptr_free_block, block_size))->size_and_tags = 
	     ((block_info*)UNSCALED_POINTER_ADD(ptr_free_block, block_size))->size_and_tags | TAG_PRECEDING_USED;
     
     // update size_and_tags of block just allocated
     ptr_free_block->size_and_tags =
             block_size | TAG_USED | ((ptr_free_block->size_and_tags) & TAG_PRECEDING_USED);
  }
...
void mm_free(void* ptr) {
  ...
  // avoid freeing already freed blocks 
  if ((!(block_to_free->size_and_tags)) & TAG_USED) {
     return;
  }

  // update size_and tags
  // update used tag of this block in header
  ...
  *((size_t*) UNSCALED_POINTER_ADD(block_to_free, (payload_size - WORD_SIZE))) = block_to_free->size_and_tags;
  // update preceding used tag of next block
  following_block->size_and_tags = following_block->size_and_tags & (~TAG_PRECEDING_USED);

  // add the given block to the free list (this updates ptrs for us)
